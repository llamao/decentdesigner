...
namespace WavTexture
{
    ...
    [CreateAssetMenu(menuName = "WavTexture")]
    public class WavTexture : ScriptableObject
    {
        public static WavTexture current;
        ...
        [SerializeField] public Texture2D[] _textures;

        public AudioClip lastSource;
        ...
        public static Texture2D BakeClip(AudioClip source, int channel, TextureFormat format)
        {
            current._channelCount = source.channels;
            current._sampleRate = source.frequency;
            current._bitRate = current.bitRate;
            current._length = source.length;
